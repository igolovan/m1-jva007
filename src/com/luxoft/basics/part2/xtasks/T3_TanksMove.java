...
public class T3_TanksMove extends JPanel
{
    ...
    {
        if(direction == 1) {
            cellYMax = tankY - cellWidth;
        } else if(direction == 2){
            cellXMax = tankX + cellWidth;
        } else if (direction == 3) {
            cellYMax = tankY + cellWidth;
        } else {
            cellXMax = tankX - cellWidth;
        ...
        System.out.println(cellYMax);
        System.out.println(cellXMax);

        while (tankX < cellXMax && tankX<576){
            tankX+=1;
            repaint();
            sleep(100);
        }
        while (tankX > cellXMax && tankX>0){
            tankX-=1;
            repaint();
            sleep(100);
        }
        while (tankY < cellYMax && tankY<576){
            tankY+=1;
            repaint();
            sleep(100);
        }
        while (tankY > cellYMax && tankY>0){
            tankY-=1;
            repaint();
            sleep(100);
        }
    ...
    final int BF_WIDTH = 576 + 30;
    final int BF_HEIGHT = 576 + 30;
